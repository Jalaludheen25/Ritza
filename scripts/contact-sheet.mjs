...
process.chdir(SP);
const urls = (await fs.readFile('all_urls.txt','utf8')).trim().split(/\r?\n/);
await fs.mkdir('thumbs',{recursive:true});
const TW=230, TH=300, COLS=8, ROWS=5, PER=COLS*ROWS;

async function grab(u,i){
  const f = `thumbs/${String(i).padStart(3,'0')}.jpg`;
...
await fs.writeFile('index.json',JSON.stringify(good));
...
for(let s=0;s<sheets;s++){
  ...
  await sharp({create:{width:W,height:H,channels:3,background:'#222'}}).composite(comps).jpeg({quality:76}).toFile(`sheet_${s}.jpg`);
  console.log('sheet_'+s+'.jpg', items.length);
}
