...
process.chdir(SP);
const urls = (await fs.readFile('cat_urls.txt','utf8')).trim().split(/\r?\n/);
await fs.mkdir('cthumbs',{recursive:true});
const TW=230, TH=300, COLS=8, ROWS=5, PER=COLS*ROWS;

async function grab(u,i){
  const f = `cthumbs/${String(i).padStart(3,'0')}.jpg`;
...
await fs.writeFile('cindex.json',JSON.stringify(good));
...
for(let s=0;s<sheets;s++){
  ...
  await sharp({create:{width:W,height:H,channels:3,background:'#222'}}).composite(comps).jpeg({quality:76}).toFile(`csheet_${s}.jpg`);
  console.log('csheet_'+s+'.jpg', items.length);
}
